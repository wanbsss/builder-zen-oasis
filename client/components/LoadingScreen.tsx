...
export default function LoadingScreen({ onComplete }: LoadingScreenProps) {
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-black transition-opacity duration-500 ${isLoading ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="absolute inset-0 opacity-10">
        ...
        <div className="absolute bottom-20 right-20 w-80 h-80 bg-neon-purple rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-neon-pink rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }} />
      </div>

      <div className="relative h-full flex items-center justify-center">
        <div className="text-center">
          {/* Logo Section */}
          <div className={`transition-all duration-1000 ${stage >= 1 ? 'scale-75 -translate-y-12' : 'scale-100'}`}>
            <div className="relative mb-8">
              ...
              <img
                ...
                className={`relative z-10 object-contain transition-all duration-1000 ${
                  stage >= 1 ? 'w-48 h-24 md:w-64 md:h-32' : 'w-64 h-32 md:w-96 md:h-48'
                } animate-float`}
              />
            </div>

            {/* Brand text */}
            <div className={`transition-all duration-1000 ${stage >= 1 ? 'opacity-80 scale-90' : 'opacity-100 scale-100'}`}>
            ...
            <div className="mt-12 animate-fade-in">
              ...
              <div className="space-y-3">
                <p className="text-red-500 text-sm font-medium animate-pulse">
                  {progress < 30 ? 'Preparing your anime journey...' :
                   progress < 60 ? 'Loading latest episodes...' :
          ...
          {stage >= 2 && (
            <div className="absolute inset-0 flex items-center justify-center animate-fade-in">
              <div className="text-center">
                ...
                <p className="text-red-500 text-lg font-medium">Loading Complete!</p>
              </div>
            </div>
          )}
        </div>
